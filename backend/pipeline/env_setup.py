...
Each graph entity becomes exactly one
agent profile. The agent's username is `<lowercased-entity-name>_<3-digit>`,
the persona is LLM-generated, and the source entity is preserved as a
back-reference so the agent can be located on the graph after the sim.

A single LLM call generates personas for the whole batch (much cheaper than
one call per entity). For very large graphs we chunk the request.
"""
...
from .models import AgentProfile, GraphEntityNode, KnowledgeGraph, make_agent_id
...
def _slugify_username(name: str) -> str:
    """Lowercased + alphanumeric only + 3-digit suffix."""
...
def _coerce_age(value, is_individual: bool) -> int:
    if not is_individual:
        return 30  # Organizations get a fixed apparent age
    try:
        v = int(value)
    except (TypeError, ValueError):
...
def _persona_dict_to_profile(
    persona: dict,
    entity: GraphEntityNode,
) -> AgentProfile:
    role = str(persona.get("role") or entity.type).strip()
    bio = str(persona.get("bio") or "").strip()
    # The LLM is asked to use \n\n as section separators inside the persona
    # string. We accept both literal newlines and the escaped form.
    ...
    return AgentProfile(
        ...
        # Apparent demographics (for the agent detail modal)
...
def _fallback_profile(entity: GraphEntityNode) -> AgentProfile:
    """Used when LLM generation fails for a particular entity slot."""
    return AgentProfile(
        id=make_agent_id(),
        user_name=_slugify_username(entity.name),
        name=entity.name,
        ...
        persona=(
            ...
            f"Behavior Profile: Speaks plainly, references concrete facts.\n\n"
            f"Unique Memory: Cites their direct experience with the question at hand.\n\n"
            f"Social Network: Connected to other entities in the same domain."
        ),
...
#
# Only entity types that represent actors (people, organizations, groups)
# should become agents. Events, ideologies, claims, evidence, and abstract
# concepts can't "speak" — and worse, asking an LLM to roleplay as
# "Nazism" or "Atomic Bombing of Nagasaki" trips Azure's content filter.

SPEAKER_TYPE_PATTERNS = frozenset((
    "person", "people", "individual", "figure", "speaker", "actor",
    "official", "leader", "expert", "scholar", "academic", "journalist",
    "witness", "survivor", "advocate", "critic", "analyst",
    # Common agentive person-suffixes (PolicyMaker, RuleMaker, BusinessOwner)
    "maker", "owner", "holder", "manager", "director", "founder",
    "chief", "head", "secretary", "minister", "judge", "writer",
    "researcher", "operator", "engineer", "designer", "artist",
    "organization", "organisation", "institution", "agency", "group",
    "community", "company", "firm", "corporation", "association",
    "outlet", "publisher", "newsroom", "press",
    "committee", "union", "party", "council", "tribunal", "court",
    "university", "school", "department", "ministry", "government",
    "ngo", "foundation", "movement", "coalition", "alliance",
    "stakeholder", "investor", "nation", "state", "republic",
))

NON_SPEAKER_TYPE_PATTERNS = frozenset((
    "event", "incident", "operation", "action", "attack", "war",
    "battle", "campaign", "bombing", "massacre", "atrocity", "crisis",
    "evidence", "document", "record", "post",
    "claim", "statement", "announcement", "speech",
    "ideology", "belief", "doctrine", "theory", "concept", "framework",
    "policy", "law", "rule", "regulation", "decision", "ruling",
    "judgment", "judgement", "verdict", "evaluation", "analysis",
    "trend", "pattern", "metric", "indicator", "measure",
    "platform", "channel", "medium", "technology",
    "place", "location", "site", "region", "city", "country",
    "date", "period", "era", "moment",
))


def _tokenize_type(type_name: str) -> list[str]:
    """Split a CamelCase or snake_case type name into ordered lowercased tokens.

    Order matters because in CamelCase the last token is usually the head
    noun (the actual category) — e.g. "MediaOutlet" is an outlet,
    "UniversityPolicy" is a policy.
    """
...
def _is_speaker_capable(entity_type: str, is_individual: bool) -> bool:
    """Decide whether an entity type can be turned into a debating agent.

    Logic:
      1. Look at the LAST token (the head noun in CamelCase). If it's a
         speaker word → speaker. If non-speaker → not a speaker. This handles
         "MediaOutlet" (outlet=speaker) vs "UniversityPolicy" (policy=non).
      2. If the head noun is unrecognised, scan all tokens. Any speaker
         token wins; otherwise any non-speaker token rejects.
      3. If still unknown, fall back to the LLM's is_individual flag.
    """
...
async def generate_agents_for_graph(
    ...
    """Generate one AgentProfile per node in the graph (capped at max_agents).

    Only nodes whose entity type can plausibly *speak* are considered.
    Events, ideologies, claims, evidence, and other abstract concepts are
    skipped. Among the speaker-capable nodes we prefer the most connected.
    """
    if not graph or not graph.nodes:
        return []

    # 1. Filter to speaker-capable nodes only
    speaker_nodes = [
        n for n in graph.nodes
        if _is_speaker_capable(n.type, n.is_individual)
    ]
    if not speaker_nodes:
        # Defensive fallback for ontologies of pure events: prefer
        # individuals, then any node at all. Better to have *some* agents
        # than to break the run.
        speaker_nodes = [n for n in graph.nodes if n.is_individual] or list(graph.nodes)

    logger.info(
        "Speaker filter: %d/%d nodes are speaker-capable",
        len(speaker_nodes), len(graph.nodes),
    )

    # 2. Sort by graph centrality (degree) so we prefer the most-connected
    ...
    for batch_start in range(0, len(selected), batch_size):
        ...
        try:
            llm = get_llm()
            # 8 personas × ~250 tokens each = ~2000 tokens of JSON.
            # Send 3500 base; Azure inflates to 6500 for reasoning headroom.

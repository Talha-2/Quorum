"""
Report generator — Stage 07 of the pipeline.
"""
...
def _fallback_outline(project: Project) -> Dict[str, Any]:
    return {
        ...
        "sections": [
            {"title": "What the agents agreed on", "description": "The shared position the swarm converged on."},
            {"title": "Where the agents disagreed", "description": "The dissents and the reasoning behind them."},
            {"title": "Key implications", "description": "What the debate suggests for next steps."},
        ],
    }
...
async def _plan_outline(project: Project) -> Dict[str, Any]:
    ...
    if project.consensus:
        ...
        dissents_summary = f"{n_dissents} dissenting agents" if n_dissents else "no dissenting agents"

    context = f"Constraints: {project.constraints}" if project.constraints else "(no extra context)"
...
def _provenance_footer(project: Project, domain: DomainProfile) -> str:
    """Deterministic provenance + (domain-supplied) disclaimer footer."""
    from quorum_backend.config import settings  # local to avoid cycles

    panel_size = len(project.agents)
    disclaimer = domain.report_provenance_disclaimer or (
        "This brief is decision support generated by a virtual review panel. "
        "The accountable humans own the final decision."
    )
    return (
        "\n---\n\n"
        "## Provenance\n\n"
